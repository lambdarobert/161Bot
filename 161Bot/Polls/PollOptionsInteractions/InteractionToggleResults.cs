...
{
    public class InteractionToggleResults
    {
        public async Task Run(SocketMessageComponent smc)
        {
            ...
            {
                var poll = PollManager.Instance.FindPollWithId(smc.Data.CustomId.Replace(InteractionPollOptions._id_interaction_toggle_results, ""));
                if(poll != null)
                {
